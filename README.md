...
```bash
python run.py exp.name={exp_name} method=mataoptnet dataset=tabula_muris lr=0.1
```

By default, method is set to MataOptNet, and dataset is set to Tabula Muris.
The experiment name must always be specified.

+ For the choice of base learners, we can modify the field `base_learner` ('SVM_CS' or 'Ridge') of `metaoptnet.yaml`.
+ For the choice of enabling the scaling factor or not, we can modify the field `scale_enabled` of `metaoptnet.yaml`.
